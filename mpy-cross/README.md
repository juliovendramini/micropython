...
The compiler is called `pybricks-mpy-cross`.  Invoke it as:

    $ ./pybricks-mpy-cross foo.py
...
the unix port of MicroPython requires the following:
    ...
    $ ./pybricks-mpy-cross -mcache-lookup-bc foo.py

Run `./pybricks-mpy-cross -h` to get a full list of options.
